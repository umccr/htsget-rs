...
pub(crate) mod tests {
  ...
  pub(crate) async fn get_decryption_keys() -> (Keys, Vec<u8>) {
  ...
  pub(crate) async fn get_encryption_keys() -> (Keys, Vec<u8>) {
    get_keys("crypt4gh/keys/alice.sec", "crypt4gh/keys/bob.pub").await
  }

  async fn get_keys(private_key: &str, public_key: &str) -> (Keys, Vec<u8>) {
