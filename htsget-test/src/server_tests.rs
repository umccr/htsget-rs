...
pub async fn test_response(response: Response, class: Class) {
  ...
  println!("{body:#?}");
  let expected_response = expected_response(class, response.expected_url_path).await;
...
pub async fn expected_response(class: Class, url_path: String) -> JsonResponse {
  ...
  let http_url = Url::new(format!("{url_path}/data/vcf/sample1-bcbio-cancer.vcf.gz"))
    .await.with_headers(Headers::new(headers));
  let urls = match class {
    Class::Header => vec![http_url.with_class(Class::Header)],
    Class::Body => vec![http_url, Url::new(expected_bgzf_eof_data_url()).await],
  };
