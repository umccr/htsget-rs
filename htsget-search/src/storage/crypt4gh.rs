...
use noodles::bam::bai::r#async::write;
    ...
    R: AsyncRead,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match ready!(item) {
            Some(Ok(buf)) => Poll::Ready(Some(Ok(DataBlockDecryptor::new(buf, this.keys, this.sender_pubkey)))),
            Some(Err(e)) => Poll::Ready(Some(Err(e))),
            None => Poll::Ready(None),
        }
    }
...
impl DataBlockDecryptor {
    fn new(src: Bytes, keys: &Keys, sender_pubkey: &Option<SenderPublicKey>) -> Self {
        Self {
            handle: tokio::task::spawn_blocking(move || DataBlockDecryptor::decrypt(src, keys, sender_pubkey))
        }
    }

    fn decrypt(src: Bytes, keys: &Keys, sender_pubkey: &Option<SenderPublicKey>) -> io::Result<PlainTextBytes> {
        let keys = Keys { method: 0, privkey: vec![], recipient_pubkey: vec![]};

        let mut read_buffer = std::io::Cursor::new(self.buf);
        let mut write_buffer = std::io::Cursor::new(vec![]);

        crypt4gh::decrypt(keys, &mut read_buffer, &mut write_buffer, 0, None, self.sender_pubkey.as_ref());

        write_buffer.into_inner()
    }
}
...
pub(crate) mod tests {
    ...
    const CIPHERTEXT: &[u8] =&[
    ];
    ...
    async fn crypt4gh_encrypt() {
        let keys = Keys{ method: 0, privkey: ALICE_SECRET_KEY, recipient_pubkey: BOB_PUBLIC_KEY }; 
        let ciphertext = DataBlockStreamDecryptor::new(PLAINTEXT, keys, Some(BOB_PUBLIC_KEY)).encrypt();
